...
| 46a | GET | `/ota/service-check` | superadmin | OTA 环境诊断（`OTA_PUBLIC_BASE_URL` / `OTA_VERIFY_BASE_URL` / 是否配置 `OTA_TOKEN`，无密钥） |
| 47 | POST | `/ota/broadcast` | superadmin | **旧** 一把梭下发（保留兼容） |
| 48 | POST | `/ota/campaigns` | superadmin | 创建新活动（`target_admins=["*"]` 全部 admin） |
| 48a | POST | `/ota/firmware/upload` | superadmin | `multipart/form-data`：`file` + `fw_version` — 仅保存 `.bin` 并 **HEAD** 诊断；**不**建活动（HEAD 失败也保留文件） |
| 48b | POST | `/ota/campaigns/from-stored` | superadmin | JSON：`filename`（`OTA_FIRMWARE_DIR` 内已有 `.bin`）、`fw_version`、`notes`、`target_admins` — **HEAD** 通过后建活动 |
| 48c | POST | `/ota/campaigns/from-upload` | superadmin | `multipart/form-data`：上传 `.bin`、**HEAD** 通过后建活动；HEAD 失败则删除已上传文件 |
