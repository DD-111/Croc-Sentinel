...
3. **看 API 日志**（实时）：
   ...
   ```
4. **健康检查**（默认 compose 将 API 映射到宿主机 **18999**，明文 HTTP；防火墙放行 **18999**）：
   ```bash
   curl -sS http://esasecure.com:18999/health
   ```
   本机：`curl -sS http://127.0.0.1:18999/health`
...
4. Smoke-test: `curl -sS http://esasecure.com:18999/health` (compose maps host **18999** → api **8088**).
...
For factory tools (`tools/factory_pack`), set **`FACTORY_UI_API_BASE=http://esasecure.com:18999`** so `GET /factory/ping` is `http://esasecure.com:18999/factory/ping`.
